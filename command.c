...
char * scommand_to_string(const scommand self){
    ...
    for (unsigned int i = 0; i < scommand_length(self); ++i){
        char * aux = g_slist_nth_data(self->list, i); //toma el comando i
        char * tmp = result; 
        result = strmerge(tmp, aux);                  //concatena el primer comando // strdup(result); o alliasing; 
        free(tmp);
    }
    if (self->out != NULL){
...
void pipeline_push_back(pipeline self, scommand sc){
    assert(self != NULL && sc != NULL);                           // Verificar que self y sc no sean nulos
    self->cmd = g_slist_append(self->cmd, sc);                    // Agregar el comando simple a la lista enlazada
    assert(!pipeline_is_empty(self));                             // Marcar que el pipeline no está vacío
}
